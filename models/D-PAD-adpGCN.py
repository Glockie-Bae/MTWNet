...
class DPAD_GCN(nn.Module):
    def __init__(self, output_len, input_len, input_dim=6, enc_hidden=1, dec_hidden=1, num_levels=3, dropout=0.5,
                 K_IMP=6, RIN=0):
...
if __name__ == '__main__':
    ...
    model = DPAD_GCN(input_len=3600, output_len=args.horizon, input_dim=6, enc_hidden=168,
                     dec_hidden=168, dropout=0.5, num_levels=2, K_IMP=6, RIN=1).cuda()

    x = torch.randn(6, 3600, 6).cuda()
